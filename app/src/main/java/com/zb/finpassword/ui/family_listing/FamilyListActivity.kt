package com.zb.finpassword.ui.family_listing
...
import com.zb.finpassword.ui.team_list.OnTeamDeleteClick
...
class FamilyListActivity : BaseActivity() , OnTeamDeleteClick, RecyclerItemTouchHelper2.RecyclerItemTouchHelperListener {
    ...
    var teamAdapter: FamilyListAdapter?=null
    ...
    companion object {
        ...
        fun getStartIntent(context: Context, category: String): Intent {
            return Intent(context, FamilyListActivity::class.java).putExtra(
                CATEGORY,
                category
            )
        }
    ...
    fun setUi() {
        ...
        progressDialog = MyCustomProgressDialog(this@FamilyListActivity)
    ...
    fun getCategoryData() {
        ...
        val call: Call<JsonObject> =
            apiService!!.getCategoryDetail(
                Prefs.getValue(this@FamilyListActivity, AppConstant.USER_ID, "").toString(),
                Prefs.getValue(this@FamilyListActivity, AppConstant.PASSWORD, "").toString(),
                Prefs.getValue(this@FamilyListActivity, AppConstant.API_KEY, "").toString(),
                "familie",
                category,
                "select"
            )
                ...
                if (response.isSuccessful()) {
                    ...
                    try{
                        ...
                        if(teamDataList.size>0){
                            rv_listing.apply {
                                teamAdapter= FamilyListAdapter(this@FamilyListActivity, teamDataList,category,subKeyList)
                                adapter = teamAdapter
                            }
                            setSwipe()
                            tv_no_data.gone()
    ...
    fun deleteItem(id: String, pos: Int,teamId:String) {
        ...
        call.enqueue(object : Callback<DeleteResponse> {
            override fun onResponse(
                ...
                if (response.isSuccessful()) {
                    showToasty(this@FamilyListActivity, response.body()?.mesage.toString(), "1")
                    Log.e("errorCode", response.body()?.mesage.toString())
                    getCategoryData()
                } else {
                    showToasty(this@FamilyListActivity, response.body()?.code.toString(), "1")
                    Log.e("errorCode", response.body()?.code.toString())
                }
            }

            override fun onFailure(call: Call<DeleteResponse>, t: Throwable) {
                showToasty(this@FamilyListActivity, t.message.toString(), "1")
    ...
    fun setSwipe() {
        ...
        val itemTouchHelperCallback: ItemTouchHelper.SimpleCallback =
            RecyclerItemTouchHelper2(0, ItemTouchHelper.LEFT, this)
    ...
    override fun onSwiped(viewHolder: RecyclerView.ViewHolder?, direction: Int, position: Int) {
        if (viewHolder is FamilyListAdapter.ItemViewHolder) {
