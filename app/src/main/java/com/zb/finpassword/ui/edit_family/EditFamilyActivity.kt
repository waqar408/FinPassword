package com.zb.finpassword.ui.edit_family
...
class EditFamilyActivity : BaseActivity() {
    ...
    companion object {
        ...
        fun getStartIntent(
            ...
            return Intent(context, EditFamilyActivity::class.java).putExtra(
                CATEGORY,
                category
            ).putExtra(
                TEAM_ID,
...
/*
            for (i in 0 until teamKeyList!!.size) {
                paramValueKeyList.add(valueKeyList.get(i))
                if (valueKeyList.get(i).toString().equals("")) {
                    showToasty(this, "Please enter valid " + keyList.get(i), "2")
                    break
                } else {
...
*/
    ...
    private fun callAddPrivacyDataApi() {
        ...
        data["userid"] = Prefs.getValue(this@EditFamilyActivity, AppConstant.USER_ID, "").toString()
        data["pwd"] = Prefs.getValue(this@EditFamilyActivity, AppConstant.PASSWORD, "").toString()
        data["key"] = Prefs.getValue(this@EditFamilyActivity, AppConstant.API_KEY, "").toString()
        data["typ"] = "familie"
        data["cat"] = category
        data["action"] = "update"
        data["version"] = "3.2"
